...
<body>
  ...
  <div class="container" id="loginForm" style="display:block;">
    ...
    <form method="post">
      ...
      <p> I Dont Have An Account ?<a href="reg.py">Sign-up</a></p>
    </form>
  </div>
</body>
</html>
...
if len(form) != 0:
    try:
        ...
        if submit_btn != 0:
            query = """ SELECT id FROM user_info WHERE Email ='%s' and Password='%s' """ % (Email,password)
            curr.execute(query)
            res = curr.fetchone()
